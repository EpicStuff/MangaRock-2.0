...
class GUI:
	...
	def __init__(self, settings: Dict, files: list[Dict]) -> None:
		...
		self.commands = {
			...
			'/save': 'save all works in tab',
			'/reupdate': 'reupdate all works in tab',
			'/reload': 'close and reopen tab (without saving)',
			'/refresh': 'redraw table, Deprecated: just reload instead',
			'/close': 'close current tab (without saving)',
			'/exit': 'exit MangaRock with save',
			'/quit': 'exit MangaRock without save',
		}
		...
		# css stuff
		ui.add_css(':root {--nicegui-default-gap: 0rem}')
		# run stuff
	...
	async def _file_opened(self, event: GenericEventArguments) -> None:
		...
		with self.panels:
			with ui.tab_panel(tab_name).style(GUI.styles.tab_panel) as tab.panel:
				...
				tab.grid = ui.aggrid(gridOptions, theme='balham', auto_size_columns=False)
...
def main(name: str, *, _dir: str | None = None, settings_file=Path('settings.taml')) -> None:  # pylint: disable=unused-argument
	...
	with rich_trace:
		gui = GUI(settings, get_files(settings))  # pylint: disable=W0612  # noqa: F841
	# start gui
...
def load_settings(settings_file: Path, _default_settings: str = default_settings) -> Dict:
	...
	def format_sites(settings_file: Path) -> None:  # puts spaces between args so that the 2nd arg of the 1st list starts at the same point as the 2nd arg of the 2nd list and so on
		# loads settings_file into file
		with settings_file.open() as f:
			file = f.readlines()
		# gets index of where 'sites:' starts
		start = next(num for num, line in enumerate(file) if line[0:6] == 'sites:')
		...
		while len(done) != len(file[start:]):
			# if all lines after and including 'sites:' are in adding then remove everything from adding
			if len(adding) + len(done) == len(file[start:]):
				adding = set()
			# for each line after and including 'sites:'
			for line_num, line in enumerate(file[start:], start):
				# not sure
				if line[0:9] == '    null:':
					done.add(line_num)
				...
				if line_num in done:
					continue
				# add line's index to done when it reaches the end
				if line[col] == '\n':
					done.add(line_num)
				# if line is in adding and next char is not ' ' then add space into line at i
				elif line_num in adding and line[col + 1] != ' ':
					file[line_num] = line[0:col] + ' ' + line[col:]
				# if elm endpoint is reached, add line into adding
				elif line[col] == ',' or (line[col] == ':' and line[col + 1:].lstrip(' ')[0] in ('&', '*', '[')):
					adding.add(line_num)
			col += 1  # increase column counter
		# write file to settings_file
